...
class Product_model extends CI_Model {
    ...
    function fetchproductdata($fetch_editid) {
        $edit_fetchdata = $this->db->select('product.id,product_title,product_description,product_category_id,product_brand_id,product_meta_keyword,product_meta_description,product_varient.sku')->join('product_varient', 'product_varient.product_id = product.id')->where('product.id', $fetch_editid)->get('product')->row_array();
    ...
    function delete_product_with_variant($product_id) {
        ...
        // delete additional market price from product_additional_price table
        $this->db->where_in('product_id', $product_id)->delete('product_additional_price');
    ...
    function delete_product_variant_by_id($id)
    {
            ...
            $this->db->where('product_variant_id', $id)->delete('product_additional_price');
            return true;
    }
    ...
    function get_product_variant_data($variant_id) {
        return $this->db->select('product.id as product_id, product_title, product_description, product_meta_keyword, product_meta_description, category_title, brand_title, product_category_id, product_varient.id as variant_id, product_varient.sku, product_varient.product_image ')->join('product', 'product.id = product_varient.product_id')->join('category', 'product.product_category_id = category.id', 'left')->join('brand', 'product.product_brand_id = brand.id', 'left')->where('product_varient.id', $variant_id)->get('product_varient')->row();
    }
    ...
    function variant_values($variantId, $productId) {
        $query = $this->db->select('id')->from('product_varient')->where('product_id', $productId)->get()->result_array();
    ...
    function get_varient_by_product_id($product_id) {
        return $this->db->where('product_id', $product_id)->get('product_varient')->row();
    }
    ...
    function save_addittional_market_data($additional_market_Data) {
        $this->db->insert_batch('product_additional_price', $additional_market_Data);
    }
    function fetchadditionalmarket($product_variant_id) {
        $edit_market = $this->db->where('product_variant_id', $product_variant_id)->get('product_additional_price')->result_array();
        return $edit_market;
    }
    function oldadditional_market_price_delete($id) {
        $this->db->where_in('product_variant_id', $id)->delete('product_additional_price');
    }
    function get_url_through_minprice($variant_id,$minprice)
    {
        $this->db->select('affiliate_marketing_url');
        $this->db->where('product_variant_id',$variant_id);
        $this->db->where('sale_price',$minprice);
        return $this->db->get('product_variant_market')->row();
    }
    function save_min_price($minprice_data)
    {
        $this->db->insert('price_history',$minprice_data);
        $minprice_insert_data = $this->db->insert_id();
        return $minprice_insert_data;
    }
    function get_min_price($variant_id) {
        return $this->db->select('*')->where('product_variant_id', $variant_id)->order_by('day',"desc")->limit(1)->get('price_history')->row();
    }
    function update_min_saleprice($minprice_data,$minprice_id)
    {
        $this->db->where('id',$minprice_id)->update('price_history',$minprice_data);
    }
    function update_user_alarm_status($variant_id,$history_id)
    {
        $this->db->query('UPDATE user_alarm SET status = 1, history_id = '.$history_id.' WHERE product_variant_id = '.$variant_id.' AND price >= (SELECT min_price FROM price_history WHERE product_variant_id = '.$variant_id.' ORDER BY day DESC LIMIT 1)');
        //UPDATE `user_alarm` SET status = 1 WHERE `product_variant_id` = 16 AND `price` <= (SELECT min_price FROM price_history WHERE product_variant_id = 16 ORDER BY day LIMIT 1)
    }
    function get_price_history_through_variantid($variant_id)
    {
        $this->db->select('price_history.*');
        $this->db->where('product_variant_id',$variant_id);
        return $this->db->get('price_history')->result_array();
    }
}