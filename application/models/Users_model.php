...
class Users_model extends CI_Model {
     ...
     * @return mixed|boolean
     */
    function create_profile($data = array(), $mail_active) 
    {
        if ($data) {
...
function reset_password_by_token($email) 
{
    ...
    function users_email_activation_exist($email)
    {
        return $this->db->where('email',$email)->get('users')->row();
    ...
    function get_user_alarm_through_userid($user_id)
    {
        $this->db->select('user_alarm.user_id,user_alarm.product_variant_id,price,product_image,product_title,product_slug,category_slug,status,(SELECT min_price FROM price_history WHERE price_history.product_variant_id = user_alarm.product_variant_id ORDER BY day DESC LIMIT 1) AS minimum_price,(SELECT day FROM price_history WHERE price_history.product_variant_id = user_alarm.product_variant_id ORDER BY day DESC LIMIT 1) AS updated_date,(SELECT affiliate_url FROM price_history WHERE price_history.product_variant_id = user_alarm.product_variant_id ORDER BY day DESC LIMIT 1) AS market_url,(SELECT day from price_history WHERE price_history.id = user_alarm.history_id) AS price_update');
        $this->db->join('product_varient','product_varient.id = user_alarm.product_variant_id','left');
        $this->db->join('product','product.id = product_varient.product_id','left');
        $this->db->join('category','category.id = product.product_category_id','left');
        $this->db->where('user_alarm.user_id',$user_id);
        return $this->db->get('user_alarm')->result();
    }
    ...
    function get_user_alarm_status($user_id)
    {
        $this->db->select('count(id) AS total_alert');
        $this->db->where('user_id',$user_id);
        $this->db->where('status',1);
        return $this->db->get('user_alarm')->row();
    }

    function update_status_through_variandid($variant_id,$user_id)
    {

        $this->db->set('status',2);
        $this->db->where('product_variant_id',$variant_id);
        $this->db->where('user_id',$user_id);
        $this->db->update('user_alarm');
        return $this->db->affected_rows();
    }
}