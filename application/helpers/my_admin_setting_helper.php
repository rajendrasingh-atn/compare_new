...
if (!function_exists('get_admin_setting')) {
    function get_admin_setting($field_name) {
        $ci = & get_instance();
        $ci->load->database();
        $menu_setting = $ci->Admin_setting_Model->get_admin_setting_by_field_name($field_name);
...
if (!function_exists('get_currency_symbol')) {
    function get_currency_symbol($currency_symbol) {
        ...
        if(class_exists('NumberFormatter')) {
            $fmt = new NumberFormatter($locale . "@currency=$currency_symbol", NumberFormatter::CURRENCY);
