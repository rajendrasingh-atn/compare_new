...
class Coupon extends Market_Controller {
    /**
     * Constructor
     */
    function __construct() 
    {
        ...
        $this->load->model('Market_couponModel');
        //load the form validation
        $this->load->library('form_validation');
        // set constants
        define('REFERRER', "referrer");
        define('THIS_URL', base_url('market/coupon'));
    ...
    function index() 
    {
        ...
        $data = $this->includes;
        $content_data = array();
        $data['content'] = $this->load->view('market/coupon/list', $content_data, TRUE);
    ...
    function add() 
    {
        ...
        else 
        {
            ...
            $coupon_content['user_id'] = $this->user['id'];
            ...
            $coupon_id = $this->Market_couponModel->insert_coupon($coupon_content);
            ...
            else
            {
             ...
             redirect(base_url('market/coupon'));
        ...
        $content_data = array();
        // load views
        $data['content'] = $this->load->view('market/coupon/form', $content_data, TRUE);
        $this->load->view($this->template, $data);
    }
    ...
    function update($coupon_id = NULL) 
    {
        ...
        $coupon_data = $this->Market_couponModel->get_coupon_by_id($coupon_id);
        ...
        else 
        {
            ...
            $language_update_status = $this->Market_couponModel->update_coupon($coupon_id, $coupon_content);
    ...
    {
        $status = $this->Market_couponModel->delete_coupon($coupon_id);
        if ($status) 
        {
    ...
    {
        $data = array();
        $list = $this->Market_couponModel->get_coupons();
        ...
        foreach ($list as $coupon_data) 
        {
            ...
            $button = '<a href="' . base_url("market/coupon/update/". $coupon_data->id) . '"  title="Edit Coupon" class="btn btn-primary btn-action mr-1">
            <i class="fas fa-pencil-alt"></i>
            </a>';

           
            $button.= '
                <a href="' . base_url("market/coupon/delete/" . $coupon_data->id) . '"  title="Delete Coupon" class="btn btn-danger btn-action mr-1 common_delete">
                <i class="fas fa-trash"></i>
                </a>';
            $row[] = $button;
            $data[] = $row;
        }
        $output = array("draw" => $_POST['draw'], "recordsTotal" => $this->Market_couponModel->count_all(), "recordsFiltered" => $this->Market_couponModel->count_filtered(), "data" => $data,);
